...
const ObjectMapIllustration: ManualIllustration = () => (
  <SvgFrame label="オブジェクト関係性マップ（カード＋カーディナリティ）図解">
    <Box x={12} y={14} w={296} h={22} text="オブジェクト関係性マップ" fill={NAVY} stroke={NAVY} textFill="#fff" fontSize={8.5} />
    {/* オブジェクトカード3枚と関係線 */}
    <Box x={20} y={58} w={80} h={34} text="顧客" stroke="#6366f1" fontSize={8.5} />
    <Box x={130} y={58} w={80} h={34} text="注文" stroke="#10b981" fontSize={8.5} />
    <Box x={230} y={118} w={80} h={34} text="商品" stroke="#f59e0b" fontSize={8.5} />
    <line x1={100} y1={75} x2={130} y2={75} stroke={BLUE} strokeWidth={1.5} markerEnd="url(#manual-arrow)" />
    <line x1={210} y1={80} x2={240} y2={118} stroke={BLUE} strokeWidth={1.5} markerEnd="url(#manual-arrow)" />
    {/* カーディナリティチップ */}
    <rect x={102} y={62} width={26} height={12} rx={6} fill={BLUE_SOFT} stroke={BLUE} />
    <text x={115} y={68} fill={BLUE} fontSize={6.5} textAnchor="middle" dominantBaseline="central">
      1:多
    </text>
    <text x={160} y={168} fill={TEXT} fontSize={8} textAnchor="middle">
      カード間の関係とカーディナリティ（1:1 / 1:多 / 多:多）を設計
    </text>
  </SvgFrame>
)

const ErDiagramIllustration: ManualIllustration = () => (
  <SvgFrame label="ER図（点線囲み＝オブジェクト＋実テーブル）図解">
    <Box x={12} y={14} w={296} h={22} text="ER図（オブジェクト × 実テーブル）" fill={NAVY} stroke={NAVY} textFill="#fff" fontSize={8.5} />
    {/* オブジェクトの点線囲み */}
    <rect x={20} y={50} width={130} height={100} rx={6} fill="none" stroke="#6366f1" strokeDasharray="5 4" />
    <text x={30} y={62} fill="#6366f1" fontSize={7.5}>
      注文（オブジェクト）
    </text>
    {/* テーブル */}
    <Box x={32} y={72} w={104} h={26} text="orders" fontSize={8} />
    <Box x={32} y={108} w={104} h={26} text="order_items" fontSize={8} />
    <Box x={190} y={80} w={104} h={26} text="products" fontSize={8} />
    {/* FKエッジ */}
    <line x1={136} y1={121} x2={190} y2={95} stroke={BLUE} strokeWidth={1.5} markerEnd="url(#manual-arrow)" />
    <text x={160} y={168} fill={TEXT} fontSize={8} textAnchor="middle">
      点線囲み＝オブジェクト、箱＝実テーブル、矢印＝FK参照
    </text>
  </SvgFrame>
)
...
export const MANUAL_ENTRIES: Record<string, ManualEntry> = {
  ...
  'io-types': {
    ...
    steps: [
      ...
      '行頭の「>」をクリックして展開し、具体データ（PDF・画像）を複数まとめてドラッグ＆ドロップで添付できます。フォルダ分けも可能です。',
      'Google Drive のファイルやフォルダは、URLを貼り付けて「リンク追加」で紐付けられます（フォルダはフォルダアイコンで表示）。',
  ...
  flows: {
    ...
    steps: [
      '「ロール追加」で担当レーンを作成し、「処理ノード追加」でノードを置いてドラッグで配置します。ツールバーで「選択／移動」を切り替え、選択中も Space＋ドラッグで画面を移動できます。',
      ...
      '「整形」で自動レイアウトします。矢印が運ぶ情報（チップ）の幅も考慮して間隔が決まります。縦／横の切り替えは向きを変えたうえで再整形されます。',
      '操作は ⌘Z で元に戻す、⌘⇧Z でやり直しできます。全画面表示（Esc で解除）も使えます。',
      'ノードの右クリックメニューから詳細フロー・プロパティ・ロール変更・削除を操作できます（ダブルクリックはプロパティ編集）。ヘッダーの「ロール」では、検索つきモーダルでレーンの表示/非表示を選べます。',
      '「共有」で閲覧専用URL（一般公開／組織メンバーのみ）を発行すると、チャット上でも劣化のないプレビューを表示できます。「PNG出力」では図全体をサイズに応じた高解像度で書き出します。',
      'フロー一覧ページでは、カードから「名前変更」「削除」と領域の割り当てができます。',
    ],
  ...
  'issue-trees': {
    ...
    steps: [
      ...
      '「共有」で閲覧専用URL（公開範囲: 一般公開／組織メンバーのみ）を発行できます。チャットに貼るとツリーのプレビューが展開されます。',
    ],
  ...
  dfd: {
    ...
    steps: [
      '第1レベルでは各業務フローがファンクション(処理)として並び、クリックすると第2レベル(ノード単位)へドリルダウンします。',
      ...
      '「PNG出力」は図全体をサイズに応じた高解像度で書き出します。',
      '「共有」で閲覧専用URL（公開範囲: 一般公開／組織メンバーのみ）を発行できます。チャットに貼ると図のプレビューが展開されます。',
    ],
  ...
  'object-map': {
    ...
    title: 'オブジェクト関係性マップ',
    purpose:
      '業務データの「オブジェクト」（DFDのデータストアと同一マスタ）を並べ、オブジェクト間の関係とカーディナリティ（1:1 / 1:多 / 多:多）を整理します。',
    steps: [
      '「DFDのデータストアから取り込み」で、第1レベルDFDのデータストアをオブジェクト化します（同名は再利用・冪等）。',
      'カードをドラッグして配置します（自動保存）。ホイールでズーム、背景ドラッグでパンできます。',
      'カードにマウスを乗せると4辺に丸ノブが出ます。ノブをドラッグして相手カードで離すと関係線が引けます（「関係線を追加」の2クリック接続も可）。',
      '関係線をクリックすると、カーディナリティ・ラベル・線形・接続辺の編集と削除ができます。',
      'カードをクリックすると右パネルで詳細編集と「所属テーブル」の付け外しができます。',
      '付箋・メモ・スコープ囲み（領域）を置けます。領域つき囲みに入れたオブジェクトは自動でその領域に紐付きます。',
      '「mermaidから生成」で erDiagram 等から一括生成できます（AIジョブ）。',
      '「共有」で閲覧専用URL（一般公開／組織のみ）を発行できます。チャット貼付でプレビューが展開されます。',
    ],
    Illustration: ObjectMapIllustration,
  },
  'er-diagram': {
    key: 'er-diagram',
    title: 'ER図',
    purpose:
      'オブジェクト（点線囲み）と実テーブル（データカタログ）を重ねて、データ構造の実装レベルを俯瞰します。FK参照はエッジで表示されます。',
    steps: [
      'オブジェクト関係性マップで設計した骨格が、点線囲み（オブジェクト）として表示されます。',
      'データカタログのテーブルは、紐づくオブジェクトの囲みの中に箱として並びます。',
      'テーブル同士のFK（外部キー）参照が矢印エッジで自動描画されます。',
      'テーブルやオブジェクトをドラッグして配置を整えます（位置は自動保存）。',
      'テーブルの中身（カラム定義）の編集はデータカタログ側で行います。',
      'オブジェクトへのテーブル紐付けは、オブジェクト関係性マップの右パネルで行います。',
    ],
    Illustration: ErDiagramIllustration,
  },
  tasks: {
    key: 'tasks',
    title: 'タスク管理',
    ...
    steps: [
      ...
      'WBS・ガント表示で日程と進捗を俯瞰します。担当者は名前から色分けされたアバターで表示されます。',
      '課題ツリーの打ち手／調査ノードから作ったタスクと相互に連携します。',
      'タスク詳細では、論点（イシューノード）・リスクに加えて GAP（課題）にも紐付けられます。',
    ],
  ...
  'tasks-gantt': {
    ...
    steps: [
      ...
      'バーやツリーの行をクリックすると右側に編集サイドバーが開き、その場で編集できます。「担当者」はステークホルダーから検索モーダルで選べます（未登録の名前も入力可）。ツリー行には担当者アバターが表示されます。',
