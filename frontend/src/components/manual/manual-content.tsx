...
export const MANUAL_ENTRIES: Record<string, ManualEntry> = {
  ...
  flows: {
    ...
    steps: [
      ...
      'ノードの右クリックメニューから「詳細フローを開く/作成」「プロパティを編集」「ロール変更」「削除」ができます（ダブルクリックはプロパティ編集）。',
      'ヘッダーの「ロール」ボタンで、レーンの表示/非表示を検索つきモーダルから選べます（担当ノードがあるレーンを外すときは再割当を案内します）。',
      '「共有」で閲覧専用URLを発行します（公開範囲: 一般公開／組織メンバーのみ）。チャットに貼ると図のプレビューが展開され、ベクタ描画なので拡大しても劣化しません。',
      '「PNG出力」は表示中の拡大率に関係なく、図全体を図のサイズに応じた高解像度で書き出します。',
